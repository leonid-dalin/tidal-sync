...
async def upload_batch_with_bisection_recovery(
    ...
    try:
        ...
        logger.bind(audit=True).error(
            "Chunk rejected, initiating bisection",
...
async def _bisect_recursive_async(
    ...
    try:
        ...
        if len(sub_chunk) == 1:
            ...
            console.print(f"  [red]❌ Dropped (Region-locked): {track_title} by {track_artist}[/red]")
            await stats.add_failed()
        else:
            # Recursive case: Split the array in half and test both sides.
            await asyncio.sleep(MEDIUM_DELAY * 2)
            mid = len(sub_chunk) // 2
            await _bisect_recursive_async(sub_chunk[:mid], upload_callback, stats, staged_tracks_map, dest_name)
            await _bisect_recursive_async(sub_chunk[mid:], upload_callback, stats, staged_tracks_map, dest_name)