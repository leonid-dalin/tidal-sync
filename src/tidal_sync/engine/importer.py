...
async def _import_tracks_async(session: tidalapi.Session, file_path: Path, stats: ImportStats,
                               is_favorites: bool = False, playlist_name: str | None = None,
    ...
    with console.status(f"[cyan]Scanning existing items in '{dest_name}'...[/cyan]"):
        if is_favorites and hasattr(user, 'favorites'):
            ...
            # Folder recreation and assignment
            if folder_name and playlist:
                folder_id = await _get_or_create_folder_async(session, folder_name)
                if folder_id:
                    await _assign_playlist_to_folder_async(session, playlist.id, folder_id)
